...
export const searchStudentFeeRefunds = async (searchQuery) => {
  try {
    const response = await axios.get(`${STUDENT_FEE_REFUND_API_ENDPOINT}search`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Token ${secureLocalStorage.getItem('token')}`
      },
...
export const deleteStudentFeeRefund = async (data) => {
  try {
    const response = await axios.delete(`${STUDENT_FEE_REFUND_API_ENDPOINT}${data.transaction_id}`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Token ${secureLocalStorage.getItem('token')}`
      },
